...
def binary_tree_zigzag_level_order_traversal(root: TreeNode) -> list[list[int]]:
    ...
    while queue:
        ...
        # Reverse the level if flag is set to False
        if not start:
            level.reverse()
    ...
    # Return the complete zigzag level order traversal
    return order
...
assert binary_tree_zigzag_level_order_traversal(root1) == [[3], [20, 9], [15, 7]]

# Example 2
root2 = create_binary_tree_from_list([1])
assert binary_tree_zigzag_level_order_traversal(root2) == [[1]]

# Example 3
root3 = create_binary_tree_from_list([])
assert binary_tree_zigzag_level_order_traversal(root3) == []

# Additional Test Cases
root4 = create_binary_tree_from_list([1, 2, 3, 4, 5, None, None])
assert binary_tree_zigzag_level_order_traversal(root4) == [[1], [3, 2], [4, 5]]

root5 = create_binary_tree_from_list([1, None, 2, None, 3])
assert binary_tree_zigzag_level_order_traversal(root5) == [[1], [2], [3]]

root6 = create_binary_tree_from_list([1, 2, 3, 4, None, None, 5])
assert binary_tree_zigzag_level_order_traversal(root6) == [[1], [3, 2], [4, 5]]

# Time Complexity: O(n)
# Space Complexity: O(n)
# Runtime: 0 ms, faster than 100.00% of Python3 online submissions
# Memory: 18.01 MB, less than 48.79% of Python3 online submissions